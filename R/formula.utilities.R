...
#'   each term (`+1` or `-1`), and its `"environment"` attribute
...
append_rhs.formula <- function(object = NULL, newterms, keep.onesided = FALSE, env = if(is.null(object)) NULL else environment(object)){
  ...
  for(i in seq_along(newterms)){
    ...
    NVL(env) <- termenv <- attr(newterms, "environment")[[i]]
...
#' `"sign"` and `"environment"`, containing, respectively a vector of
...
#' @param environment a list specifying the environments, or NULL
#' @param i list index
...
term_list <- function(x, sign, environment = NULL){
  ...
  if(!is.list(environment)) environment <- list(environment)
  structure(x,
            sign = rep(sign, length.out=length(x)),
            environment = rep(environment, length.out=length(x)),
            class = "term_list")
}

#' @describeIn term_list
#'
#' A concatenation method for `term_list`s.
#' @export
c.term_list <- function(x, ...){
  ...
  structure(
    ...
    environment = unlist(lapply(xl, attr, "environment"), use.names=FALSE),
    class = "term_list"
  )
}
...
#' @describeIn term_list
#'
#' An indexing method for `term_list`s.
#' @export
...
#'   [`term_list`]; its `environment` attribute is set to a list of
#'   `NULL`s, however.
#' 
#' @export
...
#'
#' @examples
#' stopifnot(identical(list_rhs.formula(a~b), structure(alist(b), sign=1, environment=list(globalenv()), class="term_list")))
#' stopifnot(identical(list_rhs.formula(~b), structure(alist(b), sign=1, environment=list(globalenv()), class="term_list")))
#' stopifnot(identical(list_rhs.formula(~b+NULL), structure(alist(b, NULL), sign=c(1,1), environment=rep(list(globalenv()), 2), class="term_list")))
#' stopifnot(identical(list_rhs.formula(~-b+NULL), structure(alist(b, NULL), sign=c(-1,1), environment=rep(list(globalenv()), 2), class="term_list")))
#' stopifnot(identical(list_rhs.formula(~+b-NULL), structure(alist(b, NULL), sign=c(1,-1), environment=rep(list(globalenv()), 2), class="term_list")))
#' stopifnot(identical(list_rhs.formula(~+b-(NULL+c)), structure(alist(b, NULL, c), sign=c(1,-1,-1), environment=rep(list(globalenv()), 3), class="term_list")))
#'
#' @export
list_rhs.formula<-function(object){
  ...
  structure(o, environment = rep(list(environment(object)), length(o)))
}
